...
public :
   ...
   EventDataSelector(TTree * = 0): fPosX(0), fParticlesPosX(fReader, "fParticles.fPosX"),
...
Bool_t EventDataSelector::Process(Long64_t entry)
{
   ...
   for(Int_t i=0; i<fParticlesPosX.GetSize(); ++i)
   {
      if (fParticlesMomentum[i]>40)
      {
         hPosX->Fill(fParticlesPosX[i]);
      }
   }
